(function(){
  ...
  async function loadRequests(){
    ...
    try{
      const endpoints=[
        ...
        ['/api/admin/grain-deletion-requests','Suppression grain','wheat'],
  ...
  function applySession(auth){
    ...
    const bell=document.getElementById('mtd-shell-bell');if(bell){bell.hidden=!isAdmin;bell.style.display=isAdmin?'':'none';if(isAdmin){const list=document.getElementById('mtd-shell-request-list');if(list&&!list.dataset.ready)list.innerHTML='<div class="mtd-shell-empty">Ouvrez la cloche pour charger les demandes.</div>'}else document.getElementById('mtd-shell-notifications')?.classList.remove('open')}
  }
