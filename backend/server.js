  ...
  try {
    ...
    const adminExtrasSql = isAdmin ? `
      ...
      , approvals AS (
          SELECT (
            ...
            (SELECT count(*) FROM strain_creation_requests WHERE lower(COALESCE(status::text,'pending'))='pending') +
            (SELECT count(*) FROM strain_certification_requests WHERE lower(COALESCE(status::text,'pending'))='pending')
          )::int AS total
        )
    ` : `
    ...
    return res.json({
      ...
      sources:{
        ...
        approvals:['photo_deletion_requests','petri_deletion_requests','lc_deletion_requests','grain_deletion_requests','strain_creation_requests','strain_certification_requests'],
